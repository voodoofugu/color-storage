...
function actions(
  set: (
    updater: Partial<MyStateT> | ((prev: MyStateT) => Partial<MyStateT>)
  ) => void
) {
  const self = {
    setStateWithTimeout<K extends keyof MyStateT>(
      ...
      set((prev) => {
        ...
        if (hasTimeout) clearTimeout(prev.timeouts[stateKey].id);
        ...
        return {
          ...
          timeouts: {
            ...prev.timeouts,
            [stateKey]: { id: timeoutId, originalValue },
          },
        };
      });
    },
    ...
    setNewPalette(paletteName?: string) {
      set((prev) => {
        const existingNames = prev.colorStorage.map((p) => Object.keys(p)[0]);
        const getNextAvailableName = (): string => {
          let index = prev.colorStorage.length + 1;
          while (existingNames.includes(`Palette-${index}`)) index++;
        ...
        return { colorStorage: [...prev.colorStorage, { [uniqueName]: [] }] };
      });
    },
    ...
    setNewPaletteColor(color: string) {
      ...
      set((prev) => {
        isDuplicate = prev.colorStorage.some((palette) => {
          ...
          return (
            name === Object.keys(prev.colorStorage[prev.currentPaletteId])[0] &&
      ...
      if (isDuplicate) self.setStateWithTimeout("failedColorAdding", true, 600);
    },
    ...
    setNewColorsOrder(newOrder: string[]) {
      set((prev) => ({
        colorStorage: prev.colorStorage.map((palette) => {
          const [name] = Object.entries(palette)[0];
          return name ===
            Object.keys(prev.colorStorage[prev.currentPaletteId])[0]
            ? { [name]: newOrder }
            : palette;
    ...
    clearColor(color: string) {
      set((prev) => ({
        colorStorage: prev.colorStorage.map((palette) => {
          const [name, colors] = Object.entries(palette)[0];
          return { [name]: colors.filter((c) => c !== color) };
        }),
      }));
    },

    popupOpen(content: PopupContentT | null, props?: Record<string, unknown>) {
      ...
      setTimeout(() => popupEl?.classList.remove("opening"), 200);
    },
